...
export function isSameSiteNotifyRequest(req) {
  ...
  // True same-origin browser requests: Origin host === Host (works without NEXT_PUBLIC_SITE_URL on Vercel).
  // Cross-site CSRF sends Origin of the attacker page, which will not match our Host.
  if (origin && rawHost && typeof origin === 'string' && typeof rawHost === 'string') {
    try {
